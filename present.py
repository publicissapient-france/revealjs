...
MODS = BASE + 'presentation'
TEMPLATE = BASE + 'templates/index.html'
INDEX = BASE + 'index.html'
SECTION = '''
                <section data-markdown="src/modules/{0}/slides.md.filtered"
                     data-separator="^\\n---\\n"
...
def create_html(answer):
    ...
    for module in modules:
        ...
        if os.path.exists(os.path.join(MODS, module, "slides.html")):
            with open (os.path.join(MODS, module, 'slides.html.filtered'), "r") as f:
                html=f.read()
                slides += html
        else:
...
def adjust_image_paths(answer):
    ...
    for module in modules:
        ...
        with open(os.path.join(MODS, module, current), 'r') as slides:
            file=slides.read().replace('images/', 'presentation/' + module + '/images/')
        with open(os.path.join(MODS, module, current + '.filtered'), 'w') as slides:
            slides.write(file)
